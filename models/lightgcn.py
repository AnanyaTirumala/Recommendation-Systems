#TODO: Clean this file up, remove comments.
# """
...
def normalised_adj(train_mat):
    ...
    values  = torch.from_numpy(A_norm.data)
    return torch.sparse_coo_tensor(indices, values, torch.Size(A_norm.shape))


class LightGCN(nn.Module):
    def __init__(self, n_users, n_items, emb_dim=64, n_layers=3, train_mat=None):
        super().__init__()
        self.n_users  = n_users
        self.n_items  = n_items
        self.n_layers = n_layers
        ...
        # if train_mat is not None:
        #     # self.register_buffer("adj", normalised_adj(train_mat))
        #     self.register_buffer("adj", normalised_adj(train_mat), persistent=False)
        # else:
        #     self.adj = None
        self.adj = normalised_adj(train_mat) if train_mat is not None else None


    # def _propagate(self, device):
    #     adj = self.adj.to(device)
    #     E   = torch.cat([self.user_emb.weight, self.item_emb.weight], dim=0)
    #     layer_embs = [E]
    #     for _ in range(self.n_layers):
    ...
    def _propagate(self, device):
        # adj = self.adj.to(device).coalesce()
        # # if device.type == 'cuda':
        # #     adj = adj.to_sparse_csr()
        # E = torch.cat([self.user_emb.weight, self.item_emb.weight], dim=0)
        adj = self.adj.cpu().coalesce()  # MPS has no sparse kernel; always compute on CPU
        E = torch.cat([self.user_emb.weight, self.item_emb.weight], dim=0).cpu()
        layer_embs = [E]
        for _ in range(self.n_layers):
            E = torch.sparse.mm(adj, E)
            layer_embs.append(E)
        # E_final = torch.stack(layer_embs, dim=0).mean(dim=0)
        E_final = torch.stack(layer_embs, dim=0).mean(dim=0).to(device)
    ...
    @torch.no_grad()
    def recommend(self, user_ids, top_k=10, exclude_mask=None):
        device = user_ids.device
        users_E, items_E = self._propagate(device)
        scores = users_E[user_ids] @ items_E.T
        if exclude_mask is not None:
            scores[exclude_mask] = -1e9
        _, top_items = scores.topk(top_k, dim=-1)
        return top_items